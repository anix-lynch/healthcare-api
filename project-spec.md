...
End-to-end healthcare analytics platform for hospital operations, demonstrating production-grade data engineering with clinical domain expertise. Processes 55,500+ patient encounters through a modern cloud analytics stack.
...
- **Metrics:** 85%+ AUC-ROC, SHAP interpretability
...
| **ML Model AUC-ROC** | 85%+ | `ml-pipeline/src/train.py` output |
| **Readmission Reduction** | 15% (630 cases/yr) | Model inference results |
...
|---|---|---|
| 55,500+ patient encounters | FastAPI serving real data | `api/app/database.py` |
| Star schema warehouse | dbt 6-table model | `dbt-project/models/` |
| TMDL + DAX | Power BI semantic model | `powerbi-model/` |
| 85%+ AUC-ROC ML | XGBoost trained model | `ml-pipeline/src/train.py` |
