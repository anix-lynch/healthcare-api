...
- [x] 12+ endpoints (`GET /api/encounters`, `/api/stats`, `/api/search`, etc.)
...
| 85%+ AUC-ROC ML model | `ml-pipeline/src/train.py` | ✅ |
...
- "55K real patient records, properly governed with HIPAA RLS"
- "Can run everything from CLI (no Fabric GUI debt)"
- "Proven 85%+ predictive accuracy for readmission risk"
